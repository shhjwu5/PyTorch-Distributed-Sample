...
class ClientFedAvg():
    def __init__(self,args):
        ...
        self.model = args["Model"](model_type=self.train_type,comm_type=self.communication_type).to(self.device)
        self.optimizer = args["Optimizer"](self.model.parameters(),lr=args["lr"])
        self.loss_function = args["Loss_function"].to(self.device) # .to(torch.device("cuda:0"))
